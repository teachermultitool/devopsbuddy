...
public class ForgotMyPasswordController
{
	...
	private static final String			PASSWOR_RESET_ATTRIBUTE_NAME		= "passwordReset";
			...
			ModelMap model)
	{
		if (StringUtils.isEmpty(token) || id == 0)
		{
			...
			model.addAttribute(PASSWOR_RESET_ATTRIBUTE_NAME, "false");
		...
		if (user.getId() != id)
		{
			log.warn("The user id {} passed as parameter does not match the user id {} associated with the token {}", id,
					user.getId(), token);
			model.addAttribute(PASSWOR_RESET_ATTRIBUTE_NAME, "false");
			...
			ModelMap model)
	{
		...
		{
			log.error("An unauthenticated user tried to invoke the reset passwrd POST method");
			model.addAttribute(PASSWOR_RESET_ATTRIBUTE_NAME, "false");
		...
		{
			log.error("Security breach! User {} is trying to make password reset request on behalf of", user.getId(), userId);
			model.addAttribute(PASSWOR_RESET_ATTRIBUTE_NAME, "false");
			model.addAttribute(MESSAGE_ATTRIBUTE_NAME, "You are not authorized to perfom this request.");
			return CHANGE_PASSWORD_VIEW_NAME;
		}

		userService.updateUserPassword(userId, password);
		log.info("Password successfully updated for user {}", user.getUsername());
		model.addAttribute(PASSWOR_RESET_ATTRIBUTE_NAME, "true");
		return CHANGE_PASSWORD_VIEW_NAME;
	}
}
